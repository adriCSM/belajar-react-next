...
export default function Card({ data, path }: { data: Igd; path: string }) {
  return (
    <div
    ...
    >
      <div className="w-full bg-lime-200 rounded-t-xl p-2 flex items-center justify-between ">
        <div className="text-gray-800 ">
          <p>{showFormattedDate(data.pasien.tanggal_keluar)}</p>
        </div>
        <FaCheck className="text-lime-500 rounded-full bg-white p-2 m-1  h-6 w-6" />
      </div>
      <div>
        <div className=" flex  items-center justify-between p-2">
          <p className="px-2 text-xs py-1 text-white bg-blue-500 rounded-lg ">{data.asuransi}</p>
